...
static void *poolAllocatorAllocPtr(PoolAllocator *poolAllocator) {
    ...
    if (poolAllocator->nextFree != NULL) {
        ...
        ptr = (void *) (((uintptr_t) poolAllocator->memory) +
                        (poolAllocator->maxSize - poolAllocator->unused) * poolAllocator->unitSize);
    ...
    } else // allocator is full
    {
        if (poolAllocator->ownsMemory) {
            ...
            LOG_S(ERROR) << "Pool Allocator is full!";
            return NULL;
        }
    }
...
static INLINE void poolAllocatorDealloc(PoolAllocator *poolAllocator, u32 idx) {
    poolAllocatorDeallocPtr(poolAllocator,
                            (void *) (((uintptr_t) poolAllocator->memory) + idx * poolAllocator->unitSize));
}
