...
void TextRenderer::renderText(std::string text, float x, float y, float scale,
	...
	for (c = text.begin(); c != text.end(); c++) {
		if(*c=='\n'){
			y -= float(_characters['I'].Size.y*2);
			x=xbase;
			continue;
		}
		...
		// update VBO for each character, techniquement possible avec le vertex shader
		float vertices[6][4] = { { xpos, ypos + h, 0.0f, 0.0f }, { xpos, ypos,
				0.0f, 1.0f }, { xpos + w, ypos, 1.0f, 1.0f },

		{ xpos, ypos + h, 0.0f, 0.0f }, { xpos + w, ypos, 1.0f, 1.0f }, { xpos
				+ w, ypos + h, 1.0f, 0.0f } };
